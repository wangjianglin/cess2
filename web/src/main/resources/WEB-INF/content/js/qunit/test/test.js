...
function makeurl() {
	...
	var url = testEnv.url || 'http1://example.com/search';
...
test("makeurl working",function() {
	...
	equal( makeurl(), 'http1://example.com/search?q=a%20search%20test', 'makeurl returns a default url if nothing specified in the testEnvironment');
});

module("testEnvironment with makeurl settings", {
	url: 'http1://google.com/',
	q: 'another_search_test'
});
test("makeurl working with settings from testEnvironment", function() {
	equal( makeurl(), 'http1://google.com/?q=another_search_test', 'rather than passing arguments, we use test metadata to from the url');
});
